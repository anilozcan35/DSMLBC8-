...
df = pd.read_csv("3.Modül/measurement_problems/datasets/product_sorting.csv")
print(df.shape)
df.head(10)
...
df.sort_values("rating", ascending=False).head(20) # SATIN ALMA SAYISI VE YORUM ALMA SAYILARINI GÖZ ARDI ETMİŞ OLDUK.
...
####################
# MİNMAX SCALER İLE STANDARDİZE EDİCEZ. ÇÜNKÜ DÜMDÜZ ÇARPARSAK PRUCHASE_COUNT RATİNGİ EZERDİ.
...
(df["comment_count_scaled"] * 32 / 100 +
 ...
 df["rating"] * 42 / 100) # STANDARTLIŞTIKRAN SONRA AĞIRLIKLARINI VEREREK SKOR ÜRETİYORUZ.
...
# WSS BİRDEN FAZLA FAKTÖR İLE SKORLANDIRMAK İSTERSEK KULLANILIR.
...
# PUANLARIN DAĞILIMLARINI KULLANARAK AĞIRLIKLI BİR ŞEKİLDE İSTATİSTİKSEL ORTALAMA GERÇEKLEŞTİRİR.
...
df["bar_score"] = df.apply(lambda x: bayesian_average_rating(x[["1_point",
                                                                ...
                                                                "3_point",
                                                                "4_point",
                                                                "5_point"]]), axis=1) # SUTUNLAR BAZINDA İŞLEM YAPIYOR

df.sort_values("weighted_sorting_score", ascending=False).head(20)
df.sort_values("bar_score", ascending=False).head(20)
# BAYES RATING ILE SOCIAL PROOFU KAÇIRMIŞIZ DİYEBİLİRİZ.
...
# - Weighted Rating
# - Bayesian Average Rating Score # SKORLARIN OLASIĞINA GÖRE SKORLAMAMIZI SAĞLAR
...
df = pd.read_csv("3.Modül/measurement_problems/datasets/movies_metadata.csv",
...
# SOLDAKİ KISMIN AMACI GEREKEN OY SAYISINA GÖRE BİR DÜZELTME YAPMAKTIR.
...
# (1000 / (1000+500))*8 = 5.33 # FORMÜLASYONUN SOL TARAFI
...
# (3000 / (3000+500))*8 = 6.85 #AYNI PUANA SAHİP FİLMİN PUANINI NASIL ETKİLİYOR ?
...
weighted_rating(7.40000, 11444.00000, M, C) # DEADPOOL İÇİN PUAN HESAPLIYORUZ

weighted_rating(8.10000, 14075.00000, M, C) # INCEPTION İÇİN PUAN

weighted_rating(8.50000, 8358.00000, M, C) # ESARETİN BEDELİ

df["weighted_rating"] = weighted_rating(df["vote_average"],
                                        df["vote_count"], M, C)

df.sort_values("weighted_rating", ascending=False).head(10) # IMDB STRATEJISI İLE SKOR HESAPLAMA
...
bayesian_average_rating([37128, 5879, 6268, 8419, 16603, 30016, 78538, 199430, 402518, 837905]) # GODFATHER

df = pd.read_csv("3.Modül/measurement_problems/datasets/imdb_ratings.csv")
df = df.iloc[0:, 1:]

df[["one", "two", "three", "four", "five","six", "seven", "eight", "nine", "ten"]]
