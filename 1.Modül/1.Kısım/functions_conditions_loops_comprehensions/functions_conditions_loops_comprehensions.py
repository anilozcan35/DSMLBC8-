...
#######################
# List Comprehension - Yapılan işlemi liste olarak geri döndürme - OneLineCoding geliyor aklıma.
#######################
...
[salary * 2 for salary in salaries if salary < 3000] # eğer comppherisonda sadece if varsa if sağa yazılır
[salary * 2 if salary < 3000 else salary * 0 for salary in salaries] # if ile birlikte else varsa for bölümü sağ tarafta kalır
[new_salary(salary * 2) if salary < 3000 else new_salary(salary * 0.2) for salary in salaries]
students = ["John", "Mark", "Venessa", "Mariam"]
students_no = ["John", "Venessa"]


[student.lower() if student in students_no else student.upper() for student in students] # if ve else aynı anda varsa for sağda olur
...
{k: v ** 2 for (k, v) in dictionary.items()} # keyler sabit valulerin karesi
...
{n: n ** 2 for n in numbers if n % 2 == 0} # tek bir if olduğu için sağ tarafta
...
num_cols = [col for col in df.columns if df[col].dtype != "O"] #değişkenlerden nümerik olanları alıyoruz.
soz = {}
...
df[num_cols].head() # dataframein içerisine liste yollarsam

# burası çok önemli.!!
df[num_cols].agg(new_dict) # Sert bi kullanım. gönderdiğin dictionarydeki keylerden gruplar ver valuedaki değerleri uygular.
